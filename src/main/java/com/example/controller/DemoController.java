...
import com.example.Tools.ExcelListener;
import com.example.entity.MonthTotalDto;
import com.example.entity.MonthTotalExport2Dto;
import com.example.entity.MonthTotalExportDto;
import com.example.model.auto.AttendanceSheet;
import com.example.model.auto.Fbz;
import com.example.model.auto.Jiuerwu;
import com.example.model.auto.YearMonth;
import com.example.service.IAttendanceSheetService;
import com.example.service.IFbzService;
import com.example.service.IJiuerwuService;
import com.example.service.IYearMonthService;
...
public class DemoController {
    ...
    @Autowired
    private IJiuerwuService jiuerwuService;
    @Autowired
    private IFbzService fbzService;
    @Autowired
    private IYearMonthService yearMonthService;
    ...
    public Map<String, Object> upload(@RequestParam(value = "file") MultipartFile file) throws Exception {
        ...
        result.put("code","500");
        result.put("msg", "导入失败");
        String fileName = run(file.getOriginalFilename());//获取文件名
        InputStream in = null;
        ...
        try {
            ...
            totalData.put("九二五",boxServerListener2.getData().stream().filter(m -> StringUtils.isNotEmpty(m.getCheckGroup()) && !"姓名".equals(m.getCheckGroup())).map(m -> m.getCheckGroup()).collect(Collectors.toList()));

            totalData.put("发布站",boxServerListener3.getData().stream().filter(m -> StringUtils.isNotEmpty(m.getCheckGroup()) && !"姓名".equals(m.getCheckGroup())).map(m -> m.getCheckGroup()).collect(Collectors.toList()));

            totalData.put("年月",fileName);
    ...
    @Transactional
    public void saveData(Map<String,Object> totalData) throws RuntimeException{
        ...
        String fileName = String.valueOf(totalData.get("年月"));
        //先保存年月
        String id = UUID.randomUUID().toString();
        YearMonth yearMonth = new YearMonth();
        yearMonth.setId(id).setAttendanceTime(fileName.substring(0,4)+"-"+fileName.substring(4,6));
        //校验是否已上传本月的数据
        YearMonth yearMonthData = yearMonthService.getOne(new QueryWrapper<YearMonth>().eq("attendance_time", yearMonth.getAttendanceTime()));
        if (yearMonthData != null){
            throw new RuntimeException("本月已上传考勤");
        }
        yearMonthService.save(yearMonth);

        //保存考勤汇总
        List<MonthTotalDto> monthTotal = (List<MonthTotalDto>) totalData.get("月汇总数据");
        List<AttendanceSheet> attendanceSheetList = new ArrayList<>();
        attendanceSheetList = monthTotal.stream().filter(m -> StringUtils.isNotEmpty(m.getName()) && !"姓名".equals(m.getName()) && StringUtils.isNotEmpty(m.getUserId()) && !"UserId".equals(m.getUserId())).map(
                monthTotalDto -> {
                    AttendanceSheet attendanceSheet = new AttendanceSheet();
                    BeanUtils.copyProperties(monthTotalDto, attendanceSheet);
                    attendanceSheet.setYearMonthTime(fileName.substring(0,4)+"-"+fileName.substring(4,6));
                    attendanceSheet.setYearMonthId(id);
                    return attendanceSheet;
                }
        ).collect(Collectors.toList());
        attendanceSheetService.saveBatch(attendanceSheetList);

        //保存发布站
        List<Fbz> fbzList = new ArrayList<>();
        fbzNames.forEach(item -> fbzList.add(new Fbz().setName(item).setYearMonthId(id)));
        fbzService.saveBatch(fbzList);

        //保存九二五
        List<Jiuerwu> jewList = new ArrayList<>();
        jewNames.forEach(item -> jewList.add(new Jiuerwu().setName(item).setYearMonthId(id)));
        jiuerwuService.saveBatch(jewList);
    }
    ...
    public Map<String, Object> export(HttpServletResponse response, HttpServletRequest request) throws IOException {
        Map<String, Object> result = new HashMap<>();
        result.put("code","500");
        result.put("msg", "导入失败");
        String attendanceTime = request.getParameter("attendanceTime");
        String year = attendanceTime.substring(0, 4);
        String month = attendanceTime.substring(5, 7);
        String excelName = URLEncoder.encode(year + "年" + month + "月份考勤表", "UTF-8").replaceAll("\\+", "%20");
        ...
        try {
            ...
            Map<String, List<MonthTotalExportDto>> datas = data(attendanceTime);
            WriteSheet writeSheet = EasyExcel.writerSheet(0, "九二五G"+getChineseMonth(month)+"考勤表").head(MonthTotalExportDto.class).build();
            excelWriter.write(datas.get("jew"), writeSheet);
            writeSheet = EasyExcel.writerSheet(1, "发布站"+getChineseMonth(month)+"考勤表").head(MonthTotalExport2Dto.class).build();
    ...
    private Map<String,List<MonthTotalExportDto>> data(String attendanceTime) {
        ...
        YearMonth yearMonth = yearMonthService.getOne(new QueryWrapper<YearMonth>().eq("attendance_time", attendanceTime));
        String yearMonthId = yearMonth.getId();
        //List<MonthTotalDto> monthTotal = (List<MonthTotalDto>) totalData.get("月汇总数据");
        List<AttendanceSheet> monthTotal = attendanceSheetService.queryMonthData(attendanceTime);
        //List<String> jewNames = (List<String>) totalData.get("九二五");
        List<String> jewNames = jiuerwuService.queryJew(yearMonthId);
        //jewNames.removeIf(Objects::isNull);
        //List<String> fbzNames = (List<String>) totalData.get("发布站");
        List<String> fbzNames = fbzService.queryFbz(yearMonthId);
        //fbzNames.removeIf(Objects::isNull);
        int jewNum = 1;//九二五sheet页序号
        int fbzNum = 1;//发布站sheet页序号
        //九二五
        List<AttendanceSheet> list1 = monthTotal.stream().filter(m -> m.getName() !=null && jewNames.contains(m.getName().replace("（离职）",""))).collect(Collectors.toList());
        //发布站
        List<AttendanceSheet> list2 = monthTotal.stream().filter(m -> m.getName() !=null && fbzNames.contains(m.getName().replace("（离职）",""))).collect(Collectors.toList());

        //九二五，按照给定的人名排序
        for (String str : jewNames){
            MonthTotalExportDto mt = new MonthTotalExportDto();
            for (AttendanceSheet mtd : list1){
